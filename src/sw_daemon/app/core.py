...
from sw_daemon.compositor import Display, OutputHandler, Registry
from sw_daemon.ipc import SOCKET_PATH, SocketServer
...
class SW:
    ...
    def on_global(self, name, interface, version):
        if interface == "wl_output":
            self.logger.info(f"New output detected: {name}")
            self.registry.bind_output(name, version)

    def on_global_remove(self, name):
        self.logger.info(f"Output removed: {name}")
    ...
    def remove_output_by_name(self, name):
        ...
        for handler in self.outputs:
            if str(handler.output) == str(name):
                self.logger.info(f"Cleaning up output handler for removed output: {name}")
                handler.destroy()
            else:
    ...
    def assert_initialised(self):
        ...
        for output in self.registry.outputs:
            ...
            self.logger.info(f"Creating handler for new output: {output}")
            handler = OutputHandler(
                app=self,
                output=output,
                compositor=self.registry.wl_compositor,
                layer_shell=self.registry.layer_shell,
                shm=self.registry.wl_shm,
            )
    ...
    def set_image(self, image_path):
        self.image.load_image(image_path)
        self.width, self.height = self.image.current_width, self.image.current_height
        self.logger.info(f"New image loaded: {image_path}")
