...
class OutputHandler:
    def __init__(self, app, output, compositor, layer_shell, shm):
        self.app = app
        self.output = output
        self.surface = compositor.create_surface()

        self.logger = logging.getLogger(f"sw-daemon.OutputHandler")
        self.logger.info(f"Creating layer surface for output: {self.output}")
    ...
    def on_configure(self, serial, width, height):
        self.logger.info(f"Configure event for {self.output}: {width}x{height}")
    ...
    def redraw(self):
        if self.width > 0 and self.height > 0:
            self.logger.debug(f"Redrawing output {self.output} at {self.width}x{self.height}")
            self.app.redraw_surface(self.surface, self.width, self.height)

    def destroy(self):
        self.logger.info(f"Destroying surfaces for output: {self.output}")
